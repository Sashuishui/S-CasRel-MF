...
class Framework():
    ...
    def train(self):
        ...
        def loss_fn(input, target, mask, alpha=0.2, gamma=0.1):
            if mask.shape != input.shape:
                mask = mask.unsqueeze(dim=-1)
                # 计算二元交叉熵损失
            ce_loss = F.binary_cross_entropy(input, target, reduction='none')
            # 计算 p_t，即模型预测的概率与目标值的匹配度。
            ...
            # 应用掩码并求平均
            loss = torch.sum(focal_loss * mask) / (torch.sum(mask) + 1e-8)
            return loss

        model = Casrel(self.config).to(self.device)
        optimizer = torch.optim.AdamW(model.parameters(), lr=self.config.learning_rate)
        # loss_fn = fl.FocalLoss(alpha=0.25, gamma=2)
        ...
        for epoch in range(1, self.config.epoch + 1):
            for data in tqdm(train_dataloader):
                ...
                model.zero_grad()
                torch.Size([5, 116]), torch.Size([5, 116]), torch.Size([5, 116, 50]), torch.Size([5, 116, 50])
                ...
                enhance_loss = model.orthogonal_regularizer()
                entity_loss = (subject_loss + object_loss) / 2
                loss = entity_loss + enhance_loss
                loss.backward()
                optimizer.step()
    ...
    def evaluate(self, model, dataloader):
        ...
        with torch.no_grad():
            for data in tqdm(dataloader):
                ...
                subject_features = (subject_heads + subject_tails).unsqueeze(-1).expand(text_encode.shape)
                text_encode = model.featureEnhance(text_encode, subject_features)
                subjects = []
                ...
                if len(subjects) > 0:
                    ...
                    object_heads, object_tails = model.get_special_relation_object(text_encode)
