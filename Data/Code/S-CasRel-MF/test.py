...
tokenizer = BertTokenizer.from_pretrained(con.bert_path)
model.load_state_dict(torch.load(con.save_model))

def test(model, sentence, con):
    model.eval()
    id2rel = json.load(open(con.schema, "r", encoding="utf-8"))[1]
    ...
    encoded_text = model.get_text_encode(token_ids, mask)
    pred_sub_heads, pred_sub_tails = model.get_subject(encoded_text)
    ...
    if len(subjects) > 0:
        ...
        pred_object_heads, pred_object_tails = model.get_special_relation_object(encoded_text_repeat.to(device),
                                                                                 sub_heads_mapping.to(device),
